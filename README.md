...
**Nebulla Field — Somnia Native Intent Engine とは？**

Nebulla Field は、Somnia のウォレットアクティビティを  
リアルタイムの 「Intent Field（意図のフィールド）」 として可視化するエンジンです。

ウォレットを接続するだけで  
Intent Score、最新アクション、現在のフェーズ（Predict / Synchronize / Awaiting）が  
一画面に静かに立ち上がります。

Somnia に特化した Intent レイヤーとして設計されており、  
将来的な S1 → G1 → S3 への拡張も前提にしています。

### 主なポイント

- **ウォレット接続**
  - MetaMask / Somnia Wallet / WalletConnect 対応
  - 接続時にヘッダーと 0x 入力欄が自動更新
  - ウォレットが無い場合は丁寧なメッセージを表示
- **三枚のカード**
  - PREDICT / SYNCHRONIZE / AWAITING の 3 枚で構成
  - PC では高さがきれいに揃うレイアウト
  - モバイルでも崩れないように調整
- **Intent エンジン**
  - `/api/intent/analyze` でアドレスを解析
  - Score（0〜100）やフェーズ、最新アクションを返す構造
- **Nebula ガラス UI**
  - 深いブルー〜ブラックのグラデーション
  - Glassmorphism + Framer Motion による静かな動き
  - 「Somnia 公式っぽい」高級感のある見た目
- **JSON スナップショット**
  - Intent Field 全体を JSON として書き出し可能

この README と要件にあわせて、  
Codex / AI コードアシスタントがプロジェクトを自動再構築できるような形になっています。
